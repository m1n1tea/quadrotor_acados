...
from .math_utils import quaternion_inverse, skew_symmetric, transform_trajectory, v_dot_q
from .quadrotor_model import Quadrotor3D
...
class Controller:
    def __init__(
        ...
        solver_options=None,
        acados_build_dir: str = "/tmp/quadrotor_acados",
    ):
        if q_cost is None:
        ...
        Path(acados_build_dir).mkdir(parents=True, exist_ok=True)
        ...
        for key_model in acados_models.values():
            ...
            ocp.solver_options.nlp_solver_type = (
                "SQP_RTI" if solver_options is None else solver_options.get("solver_type", "SQP_RTI")
            )

            json_file = str(Path(acados_build_dir) / f"{key_model.name}_acados_ocp.json")
    ...
    def acados_setup_model(self, nominal, model_name):
        ...
        acados_models[0] = fill_in_acados_model(x=self.x, u=self.u, p=[], dynamics=nominal, name=model_name)
    ...
    def update_trajectory(self, trajectory: np.ndarray, preferred_speed: float = None):
    ...
    def run_optimization(self, initial_state=None):
        ...
        starting_index = (
            np.argmin(np.sum((self.time_traj[self.last_closest_index :] - initial_state[:3]) ** 2, axis=1))
